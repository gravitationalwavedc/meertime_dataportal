...
import { generateFoldUrl, generateObservationUrl } from "../../helpers";

const SessionCard = ({ row }) => {
  const jname = row.jname || row.pulsar?.name;

  return (
    <Card className="mb-3 shadow-md job-card" data-testid="job-card">
      <Card.Body>
        <Card.Title>
          <span className="mr-3">{jname}</span> {row.proposalShort}
        </Card.Title>
        <Row>
          <Col md={3}>
      ...
      <Card.Footer>
        <Link
          to={generateFoldUrl(row.obsType, row.mainProject, jname)}
          className="mr-2"
          size="sm"
          variant="link"
          as={Button}
        ...
        <Link
          to={generateObservationUrl(row.mainProject, jname, row)}
          className="mr-2"
          size="sm"
          variant="link"
          as={Button}
