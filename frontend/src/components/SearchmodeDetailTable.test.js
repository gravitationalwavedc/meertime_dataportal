...
describe('the fold table component', () => {
            ...
            edges: [ 
                { 
                    node: {
                        ...
                        id: 'U2VhcmNoT2JzZXJ2YXRpb25EZXRhaWxOb2RlOjYyMzk=',
                        length: -0,
                        nant: 58,
                        nantEff: 58,
                        nbit: 8,
    ...
    it('should update the table when the project filter is changed', async () => {
        expect.hasAssertions();
        const { getAllByText, getByLabelText } = render(<SearchmodeDetailTable data={data}/>);
    ...
    it('should disable the view ephemeris button if the data is missing', () => {
        expect.hasAssertions();
        const modifiedData = { relaySearchmodeDetails: { ...data.relaySearchmodeDetails } };
        modifiedData.relaySearchmodeDetails.ephemeris = null;
        const { getByText } = render(<SearchmodeDetailTable data={modifiedData}/>);
        expect(getByText('Folding ephemeris unavailable')).toBeDisabled();
    });

    it('should toggle the ephemeris modal', () => {
        expect.hasAssertions();
        const { getByText , queryByRole } = render(<SearchmodeDetailTable data={data}/>);
