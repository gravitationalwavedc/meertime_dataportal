...
def send_verification_email(name, to, verification_code, from_email=settings.EMAIL_FROM or None):
    """
    ...
    logger.info(f"utility.py : send_verification_email subject={subject} from={from_email} to={to}")

    send_mail(subject, text_message, from_email, recipient_list=[to], html_message=html_message)


def send_password_reset_email(name, to, verification_code, from_email=settings.EMAIL_FROM or None):
    """
...
def send_activation_email(to, activation_code, from_email=settings.EMAIL_FROM or None):
    """
