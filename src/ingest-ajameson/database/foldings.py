...
class Foldings:
    ...
    def get_id(self, processing_id, folding_ephemeris_id, hdr, create=False):
        """get the id for the specified folding, creating is necessary"""
        return self._get_id(
            processing_id,
            folding_ephemeris_id,
            hdr.fold_nbin,
            hdr.fold_npol,
            hdr.fold_nchan,
            hdr.fold_dm,
    ...
    def _get_id(self, processing_id, folding_ephemeris_id, nbin, npol, nchan, dm, tsubint, create=False):
        """get the id for the specified instrument config"""
        query = GET_FOLDINGS_ID_QUERY % (processing_id, folding_ephemeris_id)
        try:
        ...
        if create and output is None:
            output = self.new(processing_id, folding_ephemeris_id, nbin, npol, nchan, dm, tsubint)
        return output
    ...
    def new(self, processing_id, folding_ephemeris_id, nbin, npol, nchan, dm, tsubint):
        query = INSERT_FOLDINGS_QUERY % (processing_id, folding_ephemeris_id, nbin, npol, nchan, dm, tsubint)
        try:
